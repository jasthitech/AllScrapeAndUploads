...
#image_clip = VideoClip(make_frame_image, duration=len(image_files))
image_clip = VideoClip(make_frame_image, duration=15)

# Concatenate the text clip and image clips
final_clip = concatenate_videoclips([text_clip, image_clip], method="compose")
#final_clip = concatenate_videoclips([text_clip] + image_clip * 5, method="compose")

# Write the video file
final_clip.write_videofile('output.mp4', codec='libx264', fps=24)
#final_clip.write_videofile('output.mp4', codec='libxvid', fps=24, rewrite=True)
